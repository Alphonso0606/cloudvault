...
const FOLDER_SUGGESTIONS = ['Travail', 'Perso', 'Photos', 'Vidéos', 'Documents', 'Archives']
...
export default function UploadModal({ onClose, onUpload, folders = [] }) {
  ...
  const startUpload = async () => {
    ...
    await Promise.all(files.map((file, i) =>
        onUpload(file, tagList, folder, (pct) => setProgresses(p => ({ ...p, [i]: pct })))
    ))
    setDone(true)
    setTimeout(onClose, 1000)
  }

  const allFolders = [...new Set([...FOLDER_SUGGESTIONS, ...folders])]
  ...
  return (
      <div style={{
        ...
        <div className="slide-up" style={{
          ...
          borderRadius: 'var(--radius-xl)', padding: '28px', width: 460, maxWidth: '90vw',
          ...
          <div
            ...
            <i className="fas fa-cloud-arrow-up" style={{
              fontSize: 32, color: dragging ? 'var(--color-accent)' : 'var(--color-text-dim)',
              display: 'block', marginBottom: 8
            }} />
            <p style={{ fontWeight: 500, marginBottom: 4 }}>Glisse tes fichiers ici</p>
              ...
              <div style={{ marginBottom: 16, maxHeight: 120, overflowY: 'auto' }}>
                {files.map((f, i) => (
                  ...
                  {(f.size / 1024 / 1024).toFixed(1)} Mo
                      ...
                      {uploading ? (
                          <div style={{ width: 50, height: 4, background: 'var(--color-bg-3)', borderRadius: 4, overflow: 'hidden', flexShrink: 0 }}>
                            <div style={{ height: '100%', borderRadius: 4, background: 'var(--color-accent)', width: `${progresses[i] || 0}%`, transition: 'width 0.3s' }} />
                          </div>
                      ) : (
                          <button onClick={() => setFiles(prev => prev.filter((_, j) => j !== i))}
                                  style={{ background: 'none', border: 'none', color: 'var(--color-text-dim)', cursor: 'pointer', fontSize: 12, flexShrink: 0 }}>
                            <i className="fas fa-times" />
                          </button>
                      )}
                    </div>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 12, color: 'var(--color-text-muted)', display: 'block', marginBottom: 6 }}>
              <i className="fas fa-folder" style={{ marginRight: 6, color: 'var(--color-warning)' }} />
              Dossier
            </label>
            <input
                value={folder} onChange={e => setFolder(e.target.value)}
                placeholder="ex: Travail, Photos, Perso..."
                style={{
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {allFolders.map(f => (
                  <button key={f} onClick={() => setFolder(f)} style={{
                    ...
                    background: folder === f ? 'var(--color-accent-dim)' : 'transparent',
                    color: folder === f ? 'var(--color-accent)' : 'var(--color-text-muted)',
                    cursor: 'pointer'
                  }}>
                    {f}
                  </button>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input
                ...
                placeholder="ex: important, 2025, urgent"
                style={{
                  width: '100%', padding: '8px 12px',
                  background: 'var(--color-bg-3)', border: '1px solid var(--color-border)',
                  borderRadius: 'var(--radius-md)', color: 'var(--color-text)', fontSize: 14, outline: 'none'
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <button onClick={startUpload} disabled={!files.length || uploading || done} style={{
              ...
              {done ? <><i className="fas fa-check" /> Terminé !</>
                  : uploading ? <><i className="fas fa-spinner" style={{ animation: 'spin 0.8s linear infinite' }} /> Upload...</>
