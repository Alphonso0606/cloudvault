export default function BottomNav({ page, onChangePage }) {
    const tabs = [
        ...
        { id: 'camera', icon: 'fa-camera',    label: 'Caméra'   },
        ...
        <nav style={{
            ...
            {tabs.map(tab => (
                <button key={tab.id} onClick={() => onChangePage(tab.id)} style={{
                    ...
                    {page === tab.id && (
                        <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 28, height: 2, background: tab.id === 'camera' ? '#ec4899' : 'var(--accent)', borderRadius: 2 }} />
                    )}
                    <i className={`fas ${tab.icon}`} style={{ fontSize: tab.id === 'camera' ? 22 : 20, marginTop: 4, color: tab.id === 'camera' && page === tab.id ? '#ec4899' : undefined }} />
                    {tab.label}
                </button>
            ))}
        </nav>
